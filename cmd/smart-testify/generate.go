...
func generateTestFuncName(method *ast.FuncDecl) (string, error) {
	...
	if method.Recv != nil && len(method.Recv.List) > 0 {
		...
		pair, err := getTypeString(method.Recv.List[0].Type)
		if err != nil {
			return "", err
		}
		if len(pair) == 0 {
...
func getTypeString(expr ast.Expr) ([]typePair, error) {
	switch t := expr.(type) {
	case *ast.Ident:
		return []typePair{{"", t.Name}}, nil
	case *ast.StarExpr:
	...
	case *ast.MapType:
		// 映射类型，递归处理键和值
		keyPairs, err := getTypeString(t.Key)
		if err != nil {
			return []typePair{}, err
		}

		valuePairs, err := getTypeString(t.Value)
		if err != nil {
			return []typePair{}, err
		}
	...
	case *ast.SelectorExpr:
		...
		if pkgIdent, ok := t.X.(*ast.Ident); ok {
			return []typePair{{pkgIdent.Name, t.Sel.Name}}, nil
		}
...
func generatePrompt(fset *token.FileSet, method *ast.FuncDecl, filePath string) (string, error) {
	...
	if method.Type.Params != nil {
		for _, param := range method.Type.Params.List {
			pairs, err := getTypeString(param.Type)
			if err != nil {
				return "", err
			}
			allPairs = append(allPairs, pairs...)
	...
	if method.Type.Results != nil {
		for _, result := range method.Type.Results.List {
			pairs, err := getTypeString(result.Type)
			if err != nil {
				return "", err
			}
			allPairs = append(allPairs, pairs...)
	...
	generatedTypeDefinationCode, err := generateTypeDefinitionCode(filePath, allPairs)
	if err != nil {
		return "", err
	}
...
6. When it involves DB error case, you can simulate DB error by closing the DB connection. For example, sqlDB, _ := db.DB() sqlDB.Close().
`,
		sourceCode,
...
func generateTypeDefinitionCode(filePath string, pairs []typePair) (string, error) {
	if len(pairs) == 0 {
		return "", nil
	}
