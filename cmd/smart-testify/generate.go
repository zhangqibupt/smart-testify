...
func parseTypeDefination(expr ast.Expr) ([]typePair, error) {
	...
	default:
		return []typePair{}, fmt.Errorf("unsupported type: %T", t)
	}
}
...
6. When it involves DB error case, you can simulate DB error by closing the DB connection. For example, sqlDB, _ := db.DB() sqlDB.Close(). In addition, you should put this case in the last to avoid affecting other cases.
7. When you need to create pointer of variable like *null.Int64 type, you can use &null.Int64{ Int64: 1, Valid: true, Set: true} to create it.
8. Be attention to the case-sensitivity of the code.
9. When DB AutoMigrate failed, you should to report the error.
10. You should not use Go keywords in the variable name. For example, you should not use "type", "package" as a variable name.
11. You should generate different cases in format like t.Run("test name", func(t *testing.T) { ... }) for each case.
12. You should use github.com/stretchr/testify/assert to do the assertion. For example, assert.Equal(t, expected, actual).
13. When it involves time.Time, you should create it by fakeTime = time.Date(2021, 1, 1, 0, 0, 0, 0, time.UTC)
14. When assert two objects equality with null.XX type fields, you should use util.util.AssertJsonEqual(t, tt.want, got) to assert them.
`,
		importSectionCode,
		methodCode,
